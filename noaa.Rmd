...
Check to see which event types are still not standardized.
* Data filtering based on values of PROPDMGEXP and CROPDMGEXP

```{r}
missing <- sum(new_df$PROPDMGEXP=="" & new_df$CROPDMGEXP=="")
```

When determining the weather events that have the most significant economic effect,  I have subsetted the data to include only records with positive values for both the property damage variable, PROPDMGEXP and the CROPDMGEXP.  This eliminates  `r missing` observations. I was aided in my analysis by reading this discussion of the coding of CROPDMGEXP and PROPDMGEXP. [https://rstudio-pubs-static.s3.amazonaws.com/58957_37b6723ee52b455990e149edde45e5b6.html]

```{r filter_damage}
damage_df <- new_df[new_df$CROPDMGEXP != "" | new_df$PROPDMGEXP != "", ]
```

In the documentation, it is noted that the units for the economic damage is coded in the variable PROPDMGEXP, so I transformed the PROPDMG variable before finding summary statistics to account for units.  The function transform takes the variables PROPDMGEXP (or CROPDMGEXP) and PROPDMG (or CROPDMG) and changes PROPDMG so that all data are denominated in the same units.  
...
damage_df$VALUE_ADJ_CROPDMG <- mapply(transform, damage_df$CROPDMGEXP, damage_df$CROPDMG)
damage_df$VALUE_ADJ_PROPDMG <- mapply(transform, damage_df$PROPDMGEXP, damage_df$PROPDMG)
```

Group by event type and year and find the average property (crop) damage.

```{r average economic damage, cache=TRUE}
library(dplyr)
economic_damage <- damage_df %>% group_by(event, YEAR) %>%              summarize(average_property_damage
                = mean(VALUE_ADJ_PROPDMG), 
                average_crop_damage = mean(VALUE_ADJ_CROPDMG),
                total_property_damage = sum(VALUE_ADJ_PROPDMG),
                total_property_damage = sum(VALUE_ADJ_PROPDMG))
```

*  Describe how the data were loaded into R and processed for analysis
*  Does the analysis include description and justification for any data           transformations?

Since different weather events affect different localities differently, I grouped by state when calculating the total number of fatalities/injuries and property damage.
